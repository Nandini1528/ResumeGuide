        ...
        <div className=' flex flex-wrap bg-blue-50 p-1 rounded-2xl border border-blue-100'>
            {tabs.map((tab) => {
                ...
                className={`relative flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm
                    ...
                    ${activeTab === tab.label ? "bg-white text-[#3276FD] shadow-lg"
                        : "text-slate-500 hover:text-[#3276FD] hover:bg-white/50"}`}
                        onClick={() => setActiveTab(tab.label)}>
                            <span className='relative z-10'>
                                {tab.label}

                                {activeTab === tab.label && (
                                    <div className=' absolute inset-0 bg-gradient-to-r from-blue-500/10 to-sky-500/10 rounded-xl'>
